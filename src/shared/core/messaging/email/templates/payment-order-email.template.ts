...
interface OrderGroup {
  ...
  groupTotal: number;
  variationId?: number;
  quantity?: number; // Add quantity field to interface
  variation?: {
    id: number;
    name: string;
    price: number;
...
interface PaymentOrderEmailContext {
  ...
  // Campos para refund emails
  isRefundEmail?: boolean;
  refundAmount?: number;
  originalTotal?: number;
  // Campos para extra quote emails
  isExtraQuoteEmail?: boolean;
}

export const getPaymentOrderEmail = (
  to: string,
  context: PaymentOrderEmailContext,
): EmailMessage => {
  const {
    customerName,
    orderNumber,
    ...
    restaurantName,
    discount ,
    calculateDiscountAfterTaxes,
    isRefundEmail = false,
    refundAmount,
    originalTotal,
    isExtraQuoteEmail = false
  } = context;
