...
function CheckoutModal(_a) {
    var product = _a.product, paymentState = _a.paymentState, onConfirm = _a.onConfirm, onClose = _a.onClose;
    var _b = (0, react_1.useState)(''), email = _b[0], setEmail = _b[1];
    var _c = (0, react_1.useState)(''), name = _c[0], setName = _c[1];
    var _d = (0, react_1.useState)('CO'), countryCode = _d[0], setCountryCode = _d[1];
    var _e = (0, react_1.useState)(''), promoCode = _e[0], setPromoCode = _e[1];
    var _f = (0, react_1.useState)({}), errors = _f[0], setErrors = _f[1];
    var step = paymentState.step, order = paymentState.order, result = paymentState.result, error = paymentState.error;
    var isProcessing = step === 'CREATING_ORDER' || step === 'PROCESSING_PAYMENT';
    var isDone = step === 'SUCCESS' || step === 'REDIRECTING' || step === 'ERROR';
    // Close on Escape
    (0, react_1.useEffect)(function () {
        var handler = function (e) { if (e.key === 'Escape' && !isProcessing)
            onClose(); };
    ...
    var price = (product.price_cents / 100).toLocaleString('es-CO', {
    ...
    return (<div style={{
      ...
      <div style={{
            ...
            boxShadow: "0 0 60px rgba(0,0,0,0.9), 0 0 40px ".concat(RARITY_GLOW[product.rarity]),
        ...
        <div style={{
          ...
          {!isProcessing && (<button onClick={onClose} style={{
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          {step === 'IDLE' && (<>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input value={name} onChange={function (e) { return setName(e.target.value); }} placeholder="Tu nombre para la factura" style={inputStyle(!!errors.name)}/>
                {errors.name && <span style={errorStyle}>{errors.name}</span>}
              </div>

              <div style={{ marginBottom: '1rem' }}>
                <label style={labelStyle}>Correo electrónico</label>
                <input type="email" value={email} onChange={function (e) { return setEmail(e.target.value); }} placeholder="tu@email.com" style={inputStyle(!!errors.email)}/>
                {errors.email && <span style={errorStyle}>{errors.email}</span>}
              </div>

              <div style={{ marginBottom: '1rem' }}>
                <label style={labelStyle}>País</label>
                <select value={countryCode} onChange={function (e) { return setCountryCode(e.target.value); }} style={__assign(__assign({}, inputStyle(false)), { appearance: 'none' })}>
                  {COUNTRIES.map(function (c) { return (<option key={c.code} value={c.code}>{c.label}</option>); })}
                </select>
              </div>
              ...
              <div style={{ marginBottom: '1.4rem' }}>
                ...
                <input value={promoCode} onChange={function (e) { return setPromoCode(e.target.value.toUpperCase()); }} placeholder="NEXUS2025" style={inputStyle(false)}/>
              </div>
          ...
          {/* ── PROCESSING ── */}
          {isProcessing && (<div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          {/* ── REDIRECTING ── */}
          {step === 'REDIRECTING' && (<div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          {/* ── SUCCESS ── */}
          {step === 'SUCCESS' && (<div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              ...
              {(result === null || result === void 0 ? void 0 : result.transactionId) && (<div style={{
          ...
          {/* ── ERROR ── */}
          {step === 'ERROR' && (<div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              <div style={{ fontSize: '2rem', marginBottom: '0.8rem' }}>🗡</div>
              <div style={{
...
var inputStyle = function (hasError) { return ({
    width: '100%',
    padding: '0.65rem 0.9rem',
    background: 'rgba(0,0,0,0.4)',
    border: "1px solid ".concat(hasError ? 'var(--color-crimson)' : 'rgba(200,134,10,0.25)'),
