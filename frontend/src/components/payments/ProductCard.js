...
function ProductCard(_a) {
    var product = _a.product, onSelect = _a.onSelect, disabled = _a.disabled;
    var cfg = RARITY_CONFIG[product.rarity];
    var price = (product.price_cents / 100).toLocaleString('es-CO', {
        style: 'currency', currency: product.currency, maximumFractionDigits: 0,
    });
    var outOfStock = product.available_stock === 0;
    var isDisabled = disabled || outOfStock;
    return (<div style={{
            background: "linear-gradient(145deg, var(--color-stone), var(--color-stone-dark))",
            border: "1px solid ".concat(cfg.border),
            boxShadow: "0 0 20px ".concat(cfg.glow),
            overflow: 'hidden',
            position: 'relative',
            transition: 'transform 0.2s, box-shadow 0.2s',
            cursor: isDisabled ? 'not-allowed' : 'pointer',
            opacity: isDisabled ? 0.6 : 1,
        }} className="product-card" onClick={function () { return !isDisabled && onSelect(product); }}>
      {/* Corner decoration */}
      ...
      <div style={{ padding: '0.9rem' }}>
        ...
        <div style={{
            height: '1px',
            background: "linear-gradient(90deg, transparent, ".concat(cfg.border, ", transparent)"),
            marginBottom: '0.7rem',
        }}/>
      ...
      <style>{"\n        .product-card:hover:not([style*=\"not-allowed\"]) {\n          transform: translateY(-3px);\n          box-shadow: 0 8px 30px rgba(0,0,0,0.5), 0 0 25px ".concat(cfg.glow, " !important;\n        }\n      ")}</style>
    </div>);
}
